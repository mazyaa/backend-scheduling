...
export const createDetailSchedule = async (
  ...
  const newDetailSchedule = await detailScheduleServices.createDetailSchedule(
    id,
    payload,
    scheduleId,
  );
...
export const getAllDetailSchedules = async (
  ...
  const scheduleId = res.locals.schedule.id;
...
export const updateDetailSchedule = async (
  req: Request,
  res: Response,
): Promise<void> => {
  const { id } = req.params;
