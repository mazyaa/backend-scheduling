...
import * as instrukturAsesorRepository from '../repositories/instruturOrAsesor';
...
export const createDetailSchedule = async (
  ...
  if (instrukturId) {
    instruktur =
      await instrukturAsesorRepository.getInstrukturOrAsesorById(instrukturId);
  }
  ...
  if (asesorId && !asesor) {
    throw new HttpError('Asesor not found', 404);
  }
  ...
  if (getLastDay !== null && getDetailSchedule !== null) {
    ...
    if (getDetailSchedule.hariKe === getLastDay && instrukturId) {
      // last day can only have asesor
      throw new HttpError(
        "Instructor can't be assigned on the last day, only assessor can be assigned",
        400,
      );
    } else if (getDetailSchedule.hariKe < getLastDay && asesorId) {
      // non last day can only have instruktur
      throw new HttpError(
        'Assessor can only be assigned on the last day, only instructor can be assigned',
        400,
      );
    }
  } else {
...
export const updateDetailScheduleById = async (
  id: string,
  payload: IDetailSchedule,
  trainingId: string,
): Promise<IDetailSchedule> => {
