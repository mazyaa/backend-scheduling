...
export const createPeserta = async (payload: any) => {
  if (payload.jadwalTrainingId) {
    const existingSchedule = await scheduleRepo.getScheduleById(payload.jadwalTrainingId);
    if (!existingSchedule) {
      throw new HttpError('Jadwal Training tidak ditemukan', 404);
    }
  }

  const newPeserta = await participantRepo.createPeserta({ ...payload });
...
export const getAllParticipant = async (query: IPaginationQuery) => {
  ...
  const [total, results] = await Promise.all([
    ...
    participantRepo.getAllParticipant({
      skip,
      take: limit,
      where,
